calc.mean.diffs.and.p.values <- function(dep_scores_DRIVE, 
  ...
  # add isDeleterious classification to each cell line
  dep_scores_DRIVE_merged <- merge(
    dep_scores_DRIVE,
    CCLE_mut_DRIVE[c("CCLE_ID","isDeleterious")],
    by="CCLE_ID")
  
  # split dataset in D (deleterious) and ND (non-deleterious)
  ...
  # do the same for Achilles without warning (no NAs in Achilles)
  ...
  # calculate the p-values for difference between samples
