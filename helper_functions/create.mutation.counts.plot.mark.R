create.mutation.counts.plot.mark <- function(dataset,dep.scores,cancer.names,
                                             CCLE.mut) {
  ...
  for (step in seq(0,gene.cut.off-interval.size,interval.size)) {
    ...
    count.table <- mutation.counts[mutation.counts$Freq > step &
  ...
  plot <- ggplot(genes.per.mut.interval.df, 
         aes(x=seq(interval.size,gene.cut.off,interval.size),y=gene.counts,
