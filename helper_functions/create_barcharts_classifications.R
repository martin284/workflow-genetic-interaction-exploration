create_barcharts_classifications <- function(DRIVE, Achilles, gene_ID) {
  ...
  barchart_DRIVE <- 
    ggplot(data=counts_class_DRIVE, aes(x=Var1, y=Freq,fill=Var1,label=Freq)) +
  ...
  barchart_Achilles <-
    ggplot(
      data=counts_class_Achilles, aes(x=Var1, y=Freq, fill=Var1,label=Freq)) +
    theme(legend.position = "none") +
    geom_bar(stat = "identity", width = 0.5) +
    ggtitle("Achilles") +
    labs(x="",y="number of cell lines") +
